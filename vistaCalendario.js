...
window.abrirModalNuevoAcontecimiento = async () => {
    ...
    // Mostramos un mensaje de carga mientras leemos el perfil del usuario para sacar sus trabajos
    extra.innerHTML = "<p style='text-align:center; color:#999;'><i class='fas fa-spinner fa-spin'></i> Preparando formulario...</p>";
    btns.innerHTML = "";
    modal.classList.remove('hidden');

    try {
        // Obtenemos los datos del usuario activo para leer sus trabajos
        const uSnap = await getDoc(doc(db, "usuarios", idActivo));
        let opcionesTrabajo = '<option value="">Selecciona qué trabajo es...</option>';
        
        if (uSnap.exists()) {
            const uData = uSnap.data();
            // Soporta que tengas guardados los trabajos en el perfil como array ("trabajos" o "empleos") o texto ("trabajo")
        ...
        // Inyectamos el formulario con las opciones y validaciones que me has pedido
        extra.innerHTML = `
            <div style="max-height: 420px; overflow-y: auto; padding-right: 5px;">
                ...
                <div id="bloque-tipo-otro" style="display: none;">
                    ...
                    <input type="text" id="event-tipo-otro" placeholder="Escribe el tipo de acontecimiento..." style="${inputStyle}" autocomplete="off">
                </div>
                ...
                <div id="bloque-fecha-normal">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <div style="flex: 1;">
                            ...
                            <input type="time" id="event-hora-inicio" style="${inputStyle}">
                        </div>
                        <div style="flex: 1;">
                            <label style="${labelStyle}">Hora de Fin *</label>
                            <input type="time" id="event-hora-fin" style="${inputStyle}">
                        </div>
                    </div>
                </div>
                ...
                <div id="bloque-fecha-viaje" style="display: none;">
                    <div style="display: flex; gap: 10px;">
                        ...
                        <div style="flex: 1;">
                            <label style="${labelStyle}">Hora Ida *</label>
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <div style="flex: 1;">
                            <label style="${labelStyle}">Hora Vuelta *</label>
        ...
        // Pre-rellenamos las fechas con el día de hoy
        ...
        extra.innerHTML = "<p style='color:red; text-align:center;'>Error al preparar el formulario.</p>";
    }
};

// Función dinámica para mostrar/ocultar los bloques dependiendo del tipo seleccionado
window.toggleCamposTipoEvento = () => {
    ...
    const bloqueOtro = document.getElementById('bloque-tipo-otro');
    ...
    // Gestión del input de "Otro"
    bloqueOtro.style.display = (tipo === "Otro") ? "block" : "none";

    // Gestión del select de "Trabajo"
    bloqueTrabajo.style.display = (tipo === "Trabajo") ? "block" : "none";
};

// Validación profunda y subida a Firebase
window.guardarNuevoAcontecimiento = async () => {
    ...
    // 1. Resolvemos el Tipo Definitivo y el Trabajo
    ...
    if (tipoSeleccionado === "Otro") {
        ...
        if (txtOtro === "") {
            errorMsg.innerText = "Por favor, especifica el tipo de acontecimiento.";
            return;
        }
        tipoFinal = txtOtro; // Para que la tarjeta muestre el nombre personalizado
    }

    if (tipoSeleccionado === "Trabajo") {
        trabajoElegido = document.getElementById('event-trabajo-select').value;
        if (!trabajoElegido) {
            errorMsg.innerText = "Por favor, selecciona a qué trabajo pertenece.";
            return;
        }
    }
    ...
    let objetoAcontecimiento = {
        titulo: titulo,
        tipo: tipoFinal, // Puede ser "Escuela", "Viaje" o lo que escriban en "Otro"
        categoria_original: tipoSeleccionado, // Guardamos la categoría madre por seguridad
        lugar: lugar,
        userId: idActivo,
    ...
    if (tipoSeleccionado === "Trabajo") {
        ...
        // Si no han escrito un lugar manual, ponemos el nombre del trabajo como lugar automáticamente
        if (lugar === "") objetoAcontecimiento.lugar = trabajoElegido; 
    }

    // 2. Resolvemos las Fechas y Horas (Obligatorias)
    if (tipoSeleccionado === "Viaje") {
        ...
        if (!fechaIda || !fechaVuelta || !horaIda || !horaVuelta) {
            errorMsg.innerText = "Para un viaje, debes indicar todas las fechas y horas de ida y vuelta.";
            return;
        }
        ...
        objetoAcontecimiento.horaIda = horaIda;
        objetoAcontecimiento.horaVuelta = horaVuelta;
        ...
        if (!horaInicio || !horaFin) {
            errorMsg.innerText = "Las horas de inicio y fin son obligatorias.";
            return;
        }
    ...
    try {
        ...
        document.getElementById('miModal').classList.add('hidden');
        renderizarCalendario(); // Se dibuja la nueva estrella en el calendario al instante
